...
export async function GET(request: Request) {
  ...
  try {
    ...
    const [vendors, orders, subscribersCount, featuredCount] = await Promise.all([
      ...
      (prisma.vendor as any).count({ where: { isSubscriber: true, active: true } }).catch(() => 0),
      (prisma.vendor as any).count({ where: { isFeatured: true, active: true } }).catch(() => 0),
    ]);
  ...
  const stats = store.getAdminStats(period);
