...
export async function POST(request: Request) {
  try {
    ...
    const slugBase = (businessName || name || 'usuario')
      .toLowerCase()
      .normalize('NFD')
      .replace(/[\u0300-\u036f]/g, '')
      .replace(/[^a-z0-9]+/g, '-')
    ...
    console.error('Registration error in DB:', err);
    return NextResponse.json({ error: 'Erro ao criar conta no banco de dados. Tente novamente.' }, { status: 500 });
  }
}
